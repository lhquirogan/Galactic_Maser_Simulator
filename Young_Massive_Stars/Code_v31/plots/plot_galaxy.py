...
fn = get_sample_data("/Users/luishenryquiroganunez/Documents/Leiden_University/3_Semester/MSc_thesis/Codes/Code_v31/plots/gala_grey.png", asfileobj=False)
img = read_png(fn)
x, y = ogrid[-20:20, -20:20]
x =np.linspace(-20.0, 20.0, 750)
y =np.linspace(-20.0, 20.0, 704)
X, Y = np.meshgrid(x, y)
#implot = plt.imshow(galaxy_grey,cmap='Greys_r',origin='lower',extent=[-20,20,-20,20])
...
cset = ax1.contourf(X, Y,galaxy_grey,1000, zdir='z', offset=0.0, cmap='Greys_r')

#ax1.plot_surface(x, y,0, rstride=5, cstride=5, facecolors=img)
segs = par.gs.segs_spiral(par.rtrunc, par.nspirplot) # why different than par.nspirseg? OJO
for j in range( len(segs) ):
    name_arm=['Norma Arm','Carina-Sagitarius Arm','Perseus Arm','Crux-Scutum Arm','Local Arm']
    #line_arm_color=['r-','b-','g-','m-','c-']
    line_arm_color=['r-','c','g-','m-','b-']
    ax1.plot( segs[j]['x'], segs[j]['y'],25, line_arm_color[j],label=name_arm[j], linewidth=3)
norma = mlines.Line2D([], [], color='r')
carina = mlines.Line2D([], [], color='b')
perseus= mlines.Line2D([], [], color='g')
crux = mlines.Line2D([], [], color='k')
...
#plt.gca().add_artist(legend3) 
ring = par.cr.central_ring(par.nspirplot) 
ax1.plot(ring[0]['x'], ring[0]['y'],25, 'm--', linewidth = 3)
#text(0.0,0.0, 'GC', fontsize=size_2)  
ax1.scatter(0.0,0.0,25,marker='+',color='m',s=20)
#plt.xlabel('X (kpc)',labelpad=-5)
#plt.ylabel('Y (kpc)',labelpad=-27)
#tick_params(axis='both',labelsize=size)
#axis([-par.rtrunc-10.0,par.rtrunc+10.0,-par.rtrunc-10.0,par.rtrunc+10.0])
...
ax1.set_xlim([-16,16])
ax1.set_ylim([-16,16])
ax1.scatter( 0, par.r0,25, marker='*', color ='y',s=10)
#plt.xticks([-10,-5,0,5,10])
#plt.yticks([-10,-5,0,5,10])
#plt.grid(True)
currentAxis = plt.gca()
...
'''
ax1.axis('off')
plt.savefig('/Users/luishenryquiroganunez/Documents/Leiden_University/3_Semester/MSc_thesis/Codes/Code_v31/plots/gala3d.png',bbox_inches='tight')  
