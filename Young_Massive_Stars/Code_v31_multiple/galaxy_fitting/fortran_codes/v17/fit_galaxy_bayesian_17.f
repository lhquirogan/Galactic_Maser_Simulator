...
c     V17 add a term to the motion uncertainties arising from uncertainty
c         in the model from parallax uncertainty
...
c     Set some maximum control values for above array dimensions
      ...
      write (lu_print,1000)
 1000 format(/' Program fit_galaxy_bayesian_17: 2016 Dec 18',//)
...
c           measured parallax error.  
 ...
 1060       format(' Parallax datum unc: input',f6.3,'; inflated to',
     +             f6.3,' mas,'
...
c           Calculate error floors for proper motion data based on 
...
cV17        Estimate and include uncertainty in the model for all motion
c           components (pm_x, pm_y, vhelio) stemming from the uncertainty 
c           in parallax distance...
            id_type = 2                      ! indicates X-motion datum
            i_d = i_d + 1
            data(i_d)          = pm_x
            data_type(i_d)     = id_type     
            source_number(i_d) = num_used    ! source number
            i_src              = source_number(i_d)
            call model_uncertainty ( params, iRC, id_type, i_src,
     +                               par, err_par, vhelio, err_v,
     +                               err_model )
            res_err(i_d)       = sqrt( err_pm_x**2 + err_pm_floor**2 +
     +                                 err_model**2 )
            write (lu_print,1067) err_model, res_err(i_d)
 1067       format(' X-motion uncertainties from parallax and total',
     +             2f7.3,' mas/yr')

            id_type = 3                      ! indicates Y-motion datum
            i_d = i_d + 1
            data(i_d)          = pm_y
            data_type(i_d)     = id_type           
            source_number(i_d) = num_used    ! source number
     ...
     +                                 err_model**2 )
            write (lu_print,1068) err_model, res_err(i_d)
 1068       format(' Y-motion uncertainties from parallax and total',
     +             2f7.3,' mas/yr')

            id_type = 4                      ! indicates Vhelio datum
            i_d = i_d + 1
            data(i_d)          = vhelio
            data_type(i_d)     = id_type           
            source_number(i_d) = num_used    ! source number
     ...
     +                               par, err_par, vhelio, err_v,
     +                               err_model )
            res_err(i_d)       = sqrt( err_v**2 + err_model**2 )       
            write (lu_print,1069) err_model, res_err(i_d)
 1069       format(' Vlsr     uncertainties from parallax and total',
     +             2f7.3,' mas/yr')
...
c           remove residual if not used in fitting
            if ( .not. use_data_type(ii) ) wtres(ii)=0.d0
...
c===================================================================
        subroutine model_uncertainty ( params, iRC, id_type, i_src,
     +                                 par, err_par, vhelio, err_v,
     +                                 err_model )

c       Calculates the variation of a motion component (specified
c       by "id_type") expected for the parallax distance associated
c       with parallax+/-uncertainty values

	implicit real*8 ( a-h, o-z )

        real*8     params(11)

c       First get expected motion value for the parallax minus uncertainty
        par_minus = par - err_par
	call calc_model ( params, iRC, id_type, i_src, 
     +                    par_minus, err_par,
     +                    vhelio, err_v,
     +                    value_minus ) 

c       Next get expected motion value for the parallax plus uncertainty
        par_plus  = par + err_par
	call calc_model ( params, iRC, id_type, i_src, 
     +                    par_plus, err_par,
     +                    vhelio, err_v,
     +                    value_plus ) 

        err_model = abs( value_plus - value_minus )/2.d0
