...
c     Set some maximum control values for above array dimensions
      ...
      write (lu_print,1000)
 1000 format(/' Huib5 fit_galaxy_bayesian_16: 2016 Dec 1',//)
...
c        Discard sources with unacceptably large fractional parallax uncertainty
         ...
         if ( par_unc_percentage.gt.par_max_unc ) then
            use_source = .false.
...
c           conversion from km/s/kpc to mas/yr...(0.211 = 1/4.74)
 ...
 1065       format(' Using proper motion err-floor of',f7.3,' mas/yr')
            ...
            source_number(i_d) = num_used    ! source number
            piadd = (pm_x / par) * err_par
c            write(*,*) pm_x, par, err_par, piadd    
            write(*,1066) res_err(i_d), piadd
 1066       format(' u_x error ',F7.3,' adding parlx unc ',F7.3)
            res_err(i_d) = sqrt(res_err(i_d)**2 + piadd**2)  
            ...
            source_number(i_d) = num_used    ! source number
            piadd = (pm_y / par) * err_par       
            write(*,1067) res_err(i_d), piadd
 1067       format(' u_y error ',F7.3,' adding parlx unc ',F7.3)
            res_err(i_d) = sqrt(res_err(i_d)**2 + piadd**2)  
