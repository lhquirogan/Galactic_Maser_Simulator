...
#-------------------------------------------------------------------------------
def full_model(file_name,conf_size=0.0,conf_alpha=0.0): #(npar,index_):    
    #-------------------------------------------------------------------------------
    ...
    if(conf_size != 0 and conf_alpha != 0):
        par = GaMePars(file_name,conf_size,conf_alpha) 
    else:
        par = GaMePars(file_name) 
    #-------------------------------------------------------------------------------
    stc = clock()
    stt = time()
    prt1='-------------------------------------------------------------------------------'
    ...
    def bar_check(xp,yp):
        ...
        large=6.6
        point = Point(xp, yp)
        p1=rotate_point(large/2.0,width/2.0,angle)
        p2=rotate_point(large/(-2.0),width/2.0,angle)
        p3=rotate_point(large/(-2.0),width/(-2.0),angle)
        p4=rotate_point(large/2.0,width/(-2.0),angle)                       
        polygon = Polygon([(p1), (p2), (p3), (p4)])
        k = polygon.contains(point)
        return k
        
    def segs_spiralxy(xp,yp,segs,ring_cent):
        #execution5.2
        ...
        #d_ring is the distance to the ring
        for i in range( len( ring_cent[0]['x'] )-1):        
            d_ring = dist_seg( xp, yp, ring_cent[0]['x'][i], ring_cent[0]['y'][i], \
                        ring_cent[0]['x'][i+1], ring_cent[0]['y'][i+1] )  
            #d_ring is the distance from xp,yp, to a tiny segment of the ring
            if debug:print 'ring_cent>>',i, d_ring
            if ( d_ring < dist_ring ):
                dist_ring = d_ring # saving d as new distance        
        ic = -1
    ...
    def draw_sample( nobj, segs,ring_cent):
        ...
        while (len(sample) < nobj):
            ...
            if (bar_check(x,y)==True):
                pxy,ic=0.9,6
                #print (x,y)              
            else:
                pxy, ic = segs_spiralxy(x, y, segs, ring_cent)
    ...
    #legend()
    ring_cent=par.cr.central_ring(par.nspirseg)
    #-------------------------------------------------------------------------------
    ...
    dsample = draw_sample( par.nobj, segs,ring_cent)
