...
class GaMePars:
    ...
    class CentralRing:
        ...
        '''
    class GalacticBar:
        """Define the central bar in the MW"""
        class SizeBar:
            def __init__(self,width,large,angle):
                self.width=width
                self.large=large
                self.angle=angle
        def __init__(self):
            """Initializes the Galactic Bar, see XXX"""
            self.bar=[]
            self.bar.append(self.SizeBar(0.5,3.5,38.0))        
        '''                                                                                                                                                                                                                      
    def __init__(self,file_name,conf_size=0.0,conf_alpha=0.0):
        """Initializes global paramters"""
        self.gs = self.GalSpiral()
        self.cr = self.CentralRing()
        #self.br = self.GalacticBar()
        self.colors = [ 'r', 'c', 'g', 'k', 'b', 'm', 'grey']
        self.nspirplot = 80                
