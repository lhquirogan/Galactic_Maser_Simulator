...
def spatial_distribution_plot(sample,par,save='no'):
    ...
    for i in range(len(xp) ):
        if (ic[i]==6):
            plot( xp[i], yp[i], '.', alpha = 0.2 , color = par.colors[ic[i]], markersize =5.0)        
        else:            
...
def Combined_plots(sample,par,save='no'):
    ...
    min_data=min(lp)
    max_data=max(lp)
    bins_data=numpy.linspace(min_data,max_data,nbar)
    #plt.gca().set_xscale("log")
    #weights=np.ones_like(f_Jyp)/len(f_Jyp)
    ofdf.hist(lp,bins=20, facecolor='b', alpha=0.5, log=True,rwidth=0.9)#,weights=weights) 
    ofdf.set_xlabel('$l$ (degrees)')
    ofdf.set_ylabel('Source Counts')
    #ofdf.set_ylim([0.9,1500])
    #ofdf.set_xlim([1E-2,1E6])
    ofdf.set_xticks(numpy.arange(-120,150,30))
    name_plot_20="long_hist.pdf"
    if(save=='yes'):
        plt.savefig(name_plot_20,bbox_inches='tight')
        #print(name_plot_2)
    plt.close()    
